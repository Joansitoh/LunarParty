...
public final class LunarClientAPI implements Listener {
    ...
    private void registerPluginChannel(String bukkitChannel) {
        ...
        messenger.registerOutgoingPluginChannel(INSTANCE, bukkitChannel);
        messenger.registerIncomingPluginChannel(INSTANCE, bukkitChannel, (channel, player, bytes) -> {
    ...
    public boolean sendPacket(Player player, LCPacket packet) {
        ...
        if (isRunningLunarClient(playerId)) {
            ...
            player.sendPluginMessage(INSTANCE, channel, LCPacket.getPacketData(packet));
